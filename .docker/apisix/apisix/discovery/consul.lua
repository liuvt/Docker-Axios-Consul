...
local function service_info()
    local host = local_conf.consul and local_conf.consul.host
    if not host then
    ...
    if local_conf.consul.prefix then
        url = url .. local_conf.consul.prefix
    end
...
local function request(request_uri, basic_auth, method, path, query, body)
    log.info("consul uri:", request_uri, ".")
    ...
    local timeout = local_conf.consul.timeout
...
local function fetch_full_registry(premature)
    ...
    local res, err = request(request_uri, basic_auth, "GET", "agent/services")
    if not res then
    ...
    local apps, err = core.json.decode(json_str)
    if not apps then
    ...
    for _, app in pairs(apps) do
        ...
        local port = app.Port
        local metadata = app.Weights

        if address and port then
            local nodes = up_apps[app.Service]
            if not nodes then
                nodes = core.table.new(#app, 0)
                up_apps[app.Service] = nodes
            end
            core.table.insert(nodes, {
                host = address,
                port = port,
                weight = metadata and metadata.Passing or default_weight,
                metadata = metadata,
            })
            if metadata then
                -- remove useless data
...
function _M.init_worker()
    if not local_conf.consul or not local_conf.consul.host or #local_conf.consul.host == 0 then
        error("do not set consul.host")
        return
    end

    local ok, err = core.schema.check(schema, local_conf.consul)
    if not ok then
        error("invalid consul configuration: " .. err)
        return
    end
    default_weight = local_conf.consul.weight or 100
    log.info("default_weight:", default_weight, ".")
    local fetch_interval = local_conf.consul.fetch_interval or 30
